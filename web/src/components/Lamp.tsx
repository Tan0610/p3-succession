...
export function Lamp({ storage, ledger }: { storage: StorageInfo | null; ledger: LedgerLine[] }) {
  ...
  return (
    <div className="lamp-wrap">
      <div className="lamp">
        <svg viewBox="0 0 120 210" role="img" aria-label={storage ? `Butter lamp: ${Math.round(days)} days of storage left` : 'Butter lamp: storage unknown'}>
          <defs>
          ...
          <text x="106" y={123 - (14 + (FLOOR / FULL) * 86)} fontSize="8" fill="#c23b30" fontFamily="var(--hand)">
            30 days
          </text>
          {storage && (
            <>
      ...
      <div>
        ...
        {low && <p className="error" style={{ marginTop: '0.8rem' }}>Below 30 days. That is trigger T3: top up now, or start a hand-off.</p>}
        {ledger.length > 0 && (
